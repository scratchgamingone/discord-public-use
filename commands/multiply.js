...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('multiply')
        .setDescription('✖️ Multiply two numbers (random if not provided)')
        .addNumberOption(option =>
            option.setName('first')
                .setDescription('The first number (optional)')
                .setRequired(false)
        )
        .addNumberOption(option =>
            option.setName('second')
                .setDescription('The second number (optional)')
                .setRequired(false)
        ),
    ...
    async execute(interaction) {
        ...
        await interaction.reply(`✖️ **${num1} × ${num2} = ${result}**`);
    }
};
